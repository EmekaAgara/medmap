  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [wRes, tRes] = await Promise.all([
        ...
        apiRequest("/wallets/me/transactions?page=1&limit=30", {
          method: "GET",
          token,
        }),
      ]);
      ...
      const pendingFundRefs = nextItems
        .filter(
          (x) => x?.status === "pending" && x?.type === "fund" && x?.reference,
        )
        .slice(0, 3)
        .map((x) => x.reference);
  ...
  const startTopUp = async () => {
    ...
    try {
      ...
      if ((pay?.paymentLink || pay?.paymentHtml) && pay?.reference) {
        ...
        Alert.alert(
          "Wallet",
          "Could not start payment. Configure Interswitch on the server.",
        );
      }
    } catch (e) {
    ...
    return (
      <SafeAreaView style={ui.screen(theme)} edges={["top"]}>
        ...
        <View style={{ gap: spacing.md }}>
          <ShimmerBlock
            theme={theme}
            style={{ height: 150, borderRadius: radii.xl }}
          />
          <ShimmerBlock
            theme={theme}
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Your ledger balance is ₦{bal.toLocaleString()}
          </Text>
        </View>

        {error ? (
          <Text style={[ui.errorText(theme), { marginBottom: spacing.sm }]}>
            {error}
          </Text>
        ) : null}
        ...
        <Text
          style={[ui.h2(theme), { fontSize: 16, marginBottom: spacing.sm }]}
        >
          Add money
        </Text>
        <View style={[ui.card(theme), { marginBottom: spacing.md }]}>
          ...
          >
            <Text style={ui.buttonTextPrimary(theme)}>
              {busy ? "Starting…" : "Top Up with Interswitch"}
            </Text>
          </TouchableOpacity>
          <Text
            style={[
          ...
          >
            You&apos;ll be redirected to Interswitch&apos;s to complete your
            payment.
          </Text>
        </View>
        ...
        {recent.length === 0 ? (
          ...
          recent.map((row) => {
            ...
            const incoming = ["fund", "transfer_in", "refund"].includes(
              row.type,
            );
                ...
                >
                  <View style={{ flex: 1, paddingRight: spacing.sm }}>
                    ...
                    <Text
                      style={[
                        ui.caption(theme),
                        { color: theme.subtleText, marginTop: 4, fontSize: 11 },
                      ]}
                    >
